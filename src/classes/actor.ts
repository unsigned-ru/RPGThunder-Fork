import { statObject} from "../interfaces";
import { clamp } from "../utils";
import cf from '../config.json';
import Discord from 'discord.js';
import { BaseBuff, DamageReductionBuff, AbsorbBuff } from "./tb_effects";
...
export abstract class Actor
{
     ...
     */
    takeDamage(damageToTake: number, execute: boolean, buffs: Discord.Collection<Actor, BaseBuff[]> | undefined, ignoreDefence = false)
    {
        ...
        if (bfs) for (let bf of bfs.filter(x => x instanceof DamageReductionBuff) as DamageReductionBuff[]) totalReduction += bf.multiplier;
        clamp(parsedDamage - parsedDamage * totalReduction, 0, Number.POSITIVE_INFINITY);
        ...
        { 
            ...
            if (bfs) for (let bf of (bfs.filter(x => x instanceof AbsorbBuff) as AbsorbBuff[]).sort((a,b) => a.duration - b.duration))
            {
    ...
    takeHealing(healingToTake: number, execute: boolean)
    {
        let hpBefore = this.hp;
        let hpToSet = clamp(this.hp+healingToTake,0,this.getStats().total.hp);
